...
public sealed class PlayToManager : IDisposable
{
    ...
    private async Task AddDevice(UpnpDeviceInfo info, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("Attempting to create PlayToController from location {Uri}", uri);
        ...
        if (controller is null)
        {
            ...
            _logger.LogInformation("DLNA Session created for {DeviceName} - {DeviceModel} using profile {ProfileName}", device.Properties.Name, device.Properties.ModelName, profile.Name);
        }
    }
